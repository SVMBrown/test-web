...
The `home-routes` handler in the `test-web.routes.home` namespace
...
The `home-page` function will in turn call the `test-web.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `test-web.handler` namespace:
...
Request middleware functions are located under the `test-web.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `test-web.dev-middleware` namespace found in
